...
const OtpInput: React.FC<OtpInputProps> = ({ length = 6, value, onChange, autoFocus = false }) => {
  ...
  const handlePaste = (pastedText: string, currentIndex: number) => {
    ...
    const digits = pastedText.replace(/[^0-9]/g, "").slice(0, length - currentIndex)
      ...
      {inputs.map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={`
              w-14 h-16 flex justify-center items-center rounded-md
